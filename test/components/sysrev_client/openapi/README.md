...
This directory contains integration tests for the Sysrev OpenAPI client, specifically testing the ability to create project sources and add articles with full text content (JSON and PDF).

## Test File

- `test_article_full_text.py` - Integration tests for adding articles with JSON and PDF full texts
...
The tests use pytest fixtures for dependency injection:

- `api_key` - Retrieves the API key from environment variables
- `api_url` - Retrieves the API URL (or uses default)
- `project_id` - Retrieves the test project ID
- `authenticated_client` - Creates an authenticated API client
- `source_id` - Creates a project source (module-scoped, shared across tests)
- `sample_pdf_data` - Generates a minimal valid PDF for testing

## Skipping Tests

If required environment variables are not set, the tests will be automatically skipped with a message indicating which variable is missing.
...
- The `source_id` fixture is module-scoped, meaning it creates one source that is shared across all tests in the module. This improves test performance and reduces API calls.
- Tests that add articles will create real articles in your Sysrev project. You may want to clean up test data after running the tests.
- The sample PDF generated in the tests is minimal but valid. For more thorough testing, you can modify the `sample_pdf_data` fixture to load an actual PDF file.
